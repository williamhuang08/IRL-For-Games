...
class BCIRL(nn.Module):
    def __init__(
        self,
        reward: DictConfig, # this is the reward function
        inner_updater: DictConfig,
        get_dataset_fn,
        batch_size: int,
        inner_opt: DictConfig,
        reward_opt: DictConfig, # optimizes the reward function
        irl_loss: DictConfig,
        plot_interval: int,
    ...
    # Manages saving/loading of model parameters
    def state_dict(self):
        return {
            **super().state_dict(),
    ...
    # Finds the reward given the current state, action, and the following action
    def viz_reward(self, cur_obs=None, action=None, next_obs=None) -> torch.Tensor:
        return self.reward(cur_obs, action, next_obs)

    # Predicts action based on the current policy, finds loss using predicted action and expert
    ...
    # Using meta learning, updates policy using reward and then updates the reward
    def update(self, policy, rollouts, logger, envs):
        ...
        if (
            ...
            self.reward_opt.step() # Update the reward
